...
def main(args):
    ...
    if args.model == 'resnet50':
        model = torchvision.models.detection.__dict__['fasterrcnn_resnet50_fpn'](num_classes=num_classes, pretrained=args.pretrained)  # 默认backbone为fasterrcnn_resnet50_fpn
    ...
    writer = SummaryWriter('runs/experiment_1')  # 您可以为不同的实验修改目录名
    ...
    if args.test_only:
        ...
        # Log AP and AR metrics for each epoch
        ap = eval_result.coco_eval['bbox'].stats[1]  # Index for AP
        ar = eval_result.coco_eval['bbox'].stats[8]  # Index for AR

        # Save metrics to a file
        with open('test_metrics.txt', 'a+') as f:
            f.write(f'500_dose, AP: {ap}, AR: {ar}\n')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        lr_scheduler.step()
        if args.output_dir and epoch % 4 == 0:
        ...
        writer.add_scalar('AR', ar, epoch)

        # Save metrics to a file
        with open('metrics.txt', 'a') as f:
            f.write(f'Epoch {epoch}, AP: {ap}, AR: {ar}\n')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', default='resnet50', help='backbone model of fasterrcnn, options are: resnet50,vgg16,alexnet')
    ...
    parser.add_argument('--resume', default='D:\\project\\phase_structure\\phase_structure_recognition\\checkpoints\\model_28.pth', help='resume from checkpoint')
    parser.add_argument('--start_epoch', default=0, type=int, help='start epoch')
    parser.add_argument('--aspect-ratio-group-factor', default=3, type=int)
    parser.add_argument(
        "--test_ori-only",
        dest="test_only",
        help="Only test_ori the model",
        action="store_true",
        default=True,
    )
    parser.add_argument(
