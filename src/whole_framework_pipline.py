...
if __name__ == "__main__":
    # -------------------- 1. 解析命令行 --------------------
    parser = argparse.ArgumentParser(description="Pipeline: image → CIF → reduced cell → relaxation")
    # A. 输入输出路径
    parser.add_argument(
        "--image",
        default="/home/aiprogram/large_area4.png",
        help="input STEM/SEM image",
    )
    parser.add_argument(
        "--outdir",
        default="/home/aiprogram/project/yaotian/phase_structure_reconstruction/pipline_result_zsm5",
        help="directory to save all intermediate results",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=500,
        help="maximum optimisation steps (default 500)",
    )
    parser.add_argument(
        "--noise",
        type=float,
        default=0.05,
        help="random noise amplitude in Å before relaxation (default 0.05 Å)",
    )
    ...
    parser.add_argument(
        "--visualize",
        action="store_true",
        help="generate the combined animation mp4",
    )
    ...
    # -------------------- 2. 图像 → 超胞 CIF --------------------
    print("\n=== Stage-1  Reconstruct super-cell from image ===")
    atoms_img, cif_super = process_image(args.image, args.outdir)

    # -------------------- 3. 递归缩减至 ≤50 原子 --------------------
    print("\n=== Stage-2  Reduce to primitive / motif (≤50 atoms) ===")
    struct = Structure.from_file(cif_super)
    print(f"Start atoms = {len(struct)}")

    for step in range(1, MAX_NUM_ITER + 1):
        if len(struct) <= MAX_ATOMS_NUM:
            print(f"Already ≤{MAX_ATOMS_NUM} atoms, stop shrinking.")
            break
        nxt = shrink_once(struct)
        if nxt is None or len(nxt) >= len(struct):
            print("No further reduction possible.")
            break
        print(f"Step {step}: {len(struct)} → {len(nxt)} atoms")
        struct = nxt

    cif_reduced = os.path.join(args.outdir, "output_final.cif")
    struct.to(filename=cif_reduced)
    print(f"Reduced cell saved to  {cif_reduced}  (atoms = {len(struct)})")

    # -------------------- 4. （可选）动画可视化 --------------------
    if args.visualize:
        print("\n=== Stage-3  Create animation ===")
        visualize_results(args.image, cif_super)

    # -------------------- 5. 结构弛豫 --------------------
    print("\n=== Stage-4  MatterSim relaxation ===")
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"Running MatterSim on {device}")

    # 选择待弛豫结构：命令行 -f 优先，否则用刚缩减好的单胞
    relax_input = cif_reduced
    try:
        atoms_relax = read(relax_input)
        axis, atoms_relax = add_noise_on_axis(atoms_relax, noise_scale=args.noise)
        print(
            f"Loaded {relax_input} (atoms={len(atoms_relax)}) "
            f"and added noise on {['x','y','z'][axis]} axis"
        )
    except Exception as err:
        raise SystemExit(f"Cannot read {relax_input}: {err}")

    atoms_relax.calc = MatterSimCalculator(
    ...
    converged, atoms_relaxed = relaxer.relax(atoms_relax, steps=args.steps)
    relaxed_path = os.path.join(args.outdir, "relaxed.cif")
    write(relaxed_path, atoms_relaxed)
    print(f"Relaxation {'converged' if converged else 'not converged'} → saved to {relaxed_path}")

    # -------------------- 6. 打印能量 & 应力 --------------------
    print("\n=== Final properties of relaxed structure ===")
    print(f"Energy (eV)                 = {atoms_relaxed.get_potential_energy():.6f}")
    print(f"Energy per atom (eV/atom)   = {atoms_relaxed.get_potential_energy() / len(atoms_relaxed):.6f}")
    print(f"Forces of first atom (eV/Å) = {atoms_relaxed.get_forces()[0]}")
    sxx = atoms_relaxed.get_stress(voigt=False)[0][0]
    print(f"Stress[0][0] (eV/Å³)        = {sxx:.6f}")
    print(f"Stress[0][0] (GPa)          = {sxx / GPa:.6f}")
